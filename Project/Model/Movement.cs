...
namespace Droid_financial
{
    public class Movement : ICloneable
    {
        ...
        private List<int> _userIdParticipant;
        ...
        public List<int> UserIdParticipant
        {
        ...
        public Movement()
        {
            ...
            _userIdParticipant = new List<int>();
            ...
            for (int i = 0; i < node.AttributeCount; i++)
            {
                try
                {
                    ...
                    switch (node.Name.ToLower())
                    {
                        ...
                        case "userid": foreach (string item in node.Value.Split(';')) this.UserId.Add(int.Parse(item)); break;
                        case "useridparticipant": foreach (string item in node.Value.Split(';')) this.UserIdParticipant.Add(int.Parse(item)); break;
                    }
                }
                catch (Exception)
                {
        ...
        public string ToXml()
        {
            ...
            foreach (int item in _userId) lstUser += item + ";";
            if (lstUser.EndsWith(";")) strUser = lstUser.Substring(0, lstUser.Length - 1);

            foreach (int item in _userIdParticipant) lstUserParticipant += item + ";";
        ...
        public float Convert(Change chg, string targetCurrency)
        {
        ...
        public object Clone()
        {
            ...
            mvt.Amount = _amount;
            mvt.Name = _name;
            mvt.UserId = new List<int>(_userId);
            mvt.Gop = _gop;
            ...
            mvt.UserIdParticipant = new List<int>(_userIdParticipant);
